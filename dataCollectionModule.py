...
global imgList, leftMotorList, rightMotorList
countFolder = 0
count = 0
imgList = []
leftMotorList = []
...
def saveData(img, leftMotor, rightMotor):
    global imgList, leftMotorList, rightMotorList
    now = datetime.now()
    ...
    leftMotorList.append(leftMotor)
    rightMotorList.append(rightMotor)

# SAVE LOG FILE WHEN THE SESSION ENDS
def saveLog():
    global imgList, leftMotorList, rightMotorList
    rawData = {'Image': imgList,
                'LeftMotor': leftMotorList,
                'RightMotor': rightMotorList}
